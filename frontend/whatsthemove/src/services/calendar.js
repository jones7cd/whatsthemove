...
import { calendar } from "@/services/feathers";
 ...
 * 
 * @param  {object} calendar The course to be created
 * @return {object}        The course that was created
 */
export const addEvent = async calendar => await calendar.create(calendar);
 ...
 */
export const getCalender = async query => {
 ...
 * 
 * @param  {number} id     The ID of the course to be retrieved
 * @param  {object} calendar The course data to be updated
 * @param  {object} params Any additional feathers query params
 * @return {object}        The updated course object
 */
export const updateEvent = async (id, course, params) => {
  // make sure a course ID was specified
  if ("undefined" === typeof id) {
    throw new Error("Please provide a valid course ID.");
  }
  params = params || {};
  return await calendar.patch(id, calendar, params);
}